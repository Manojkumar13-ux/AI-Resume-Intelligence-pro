...
function Dashboard() {
  ...
  const handleUploadAndAnalyze = async () => {
    ...
    // ✅ Direct upload without saving to localStorage
    ...
    try {
      ...
      const uploadResponse = await fetch('http://localhost:5001/api/resume/upload', {
        method: 'POST',
        headers: {
      ...
      if (uploadResponse.ok && uploadData.success) {
        const analyzeResponse = await fetch(`http://localhost:5001/api/resume/${uploadData.resumeId}/analyze`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
        ...
        if (analyzeResponse.ok && analyzeData.success) {
          ...
          // ✅ Navigate to analysis page with fresh data
          navigate('/analysis', { 
            state: { 
      ...
      alert('Error uploading/analyzing resume');
      setUploading(false);
    }
  };
